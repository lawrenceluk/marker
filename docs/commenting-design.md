...
Long-press/select text, adjust handles, then tap the small speech-bubble icon immediately right of the bottom-right-most rendered selection line. The icon is 24 px on desktop with a 4 px gap and centered on the line; touch uses 28 px with a 14 px gap to clear selection handles. It flips left/down only for viewport overflow or a detectable fixed/absolute overlay. It stays absent during primary-pointer drags, appears after mouse release, and waits for 350 ms without selection changes after touch release or native handle adjustments. Keyboard selections debounce for 140 ms after key release. New gestures, selection changes, scrolling and collapsed selections hide it; hiding synchronously disables hit testing before removal. It preserves the native selection callout and does not intercept document touch gestures. The compact composer focuses synchronously inside that tap, allowing iOS to open its keyboard without another field tap. Type and send with the icon. On desktop, Enter or Cmd/Ctrl+Enter sends and Shift+Enter inserts a newline; mobile Return inserts a newline. Composition/IME Enter never submits prematurely.
